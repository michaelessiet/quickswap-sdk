...
import { ChainId, QuickswapContractFactoryPublic, WETH } from '../..';
import { MOCK1INCH } from '../../mocks/1inch-token.mock';
import { MOCK_PROVIDER_URL } from '../../mocks/provider-url.mock';

describe('QuickswapContractFactoryPublic', () => {
  const quickswapContractFactoryPublic = new QuickswapContractFactoryPublic(
    ChainId.MATIC, MOCK_PROVIDER_URL()
  );

  it('allPairs', async () => {
    const result = await quickswapContractFactoryPublic.allPairs('0x01');
    expect(result).toEqual('0xbB60D1f9cbAeFA6845bf635db23Ea25a27A11E83');
  });

  it('allPairsLength', async () => {
    const result = await quickswapContractFactoryPublic.allPairsLength();
    expect(isHexString(result)).toEqual(true);
  });

  it('createPair', () => {
    const result = quickswapContractFactoryPublic.createPair(
  ...
  it('feeTo', async () => {
    const result = await quickswapContractFactoryPublic.feeTo();
    expect(isHexString(result)).toEqual(true);
  });

  it('feeToSetter', async () => {
    const result = await quickswapContractFactoryPublic.feeToSetter();
    expect(isHexString(result)).toEqual(true);
  });

  it('getPair', async () => {
    const result = await quickswapContractFactoryPublic.getPair(
      WETH.MATIC().contractAddress,
      MOCK1INCH().contractAddress
    );
  ...
  it('setFeeTo', async () => {
    const result = await quickswapContractFactoryPublic.setFeeTo(
  ...
  it('setFeeToSetter', async () => {
    const result = await quickswapContractFactoryPublic.setFeeToSetter(
      '0x05B0c1D8839eF3a989B33B6b63D3aA96cB7Ec142'
    );
    expect(result).toEqual(
